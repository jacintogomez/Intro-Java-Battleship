	            ...
	            for (int x = 0; x < 10; x++) {
	                for (int y = 0; y < 10; y++) {
	                    ...
	                    if(corner=='n') {
	                    	...
	                    	if(corner=='l') {
		                    	g.fillArc((x * width)+xoffset+cellwidth/2, (y * height)+yoffset, width, height,90,180);
	                    	}
	                    	if(corner=='r') {
		                    	g.fillArc((x * width)+xoffset-cellwidth/2, (y * height)+yoffset, width, height,270,180);
	                    	}
	                    	if(corner=='t') {
		                    	g.fillArc((x * width)+xoffset, (y * height)+yoffset+cellwidth/2, width, height,0,180);
	                    	}
	                    	if(corner=='b') {
		                    	g.fillArc((x * width)+xoffset, (y * height)+yoffset-cellwidth/2, width, height,180,180);
	                    	}
	                    }
	                }
	            }
			...
			public void paintComponent(Graphics g) {
	            ...
	            for (int x = 0; x < 10; x++) {
	                for (int y = 0; y < 10; y++) {
	                	...
	                	g.setColor(gettilecolor(opgrid[x][y],true));
	                    if(corner=='n') {
	                    	g.fillRect((x * width)+xoffset+1, (y * height)+yoffset+1, width-2, height-2);
	                    }else {
	                    	g.setColor(Color.BLUE);
